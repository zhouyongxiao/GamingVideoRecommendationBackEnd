...
public class ItemRecommender {
    ...
    // Types are one of [Stream, Video, Clip]. Add items
    // are related to the top games provided in the argument
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames)
        ...
        for (Game game : topGames) {
            List<Item> items;
            try {
                items = client.searchByType(game.getId(),
                        type, DEFAULT_PER_GAME_RECOMMENDATION_LIMIT);
...
//                    recommendedItems.size() > 0 && items.size() > 0) {
//                recommendedItems.add(items.get(0));
//                items.remove(0);
//            }
//
//            if (recommendedItems.size() >= DEFAULT_TOTAL_RECOMMENDATION_LIMIT) {
...
//            }
    ...
    // All items are related to the items previously favorited
    // by the user. E.g., if a user favorited some videos about
    // game "Just Chatting", then it will return some other
    // videos about the same game.

    private List<Item> recommendByFavoriteHistory(
            Set<String> favoriteItemIds,
            List<String> favoriteGameIds, ItemType type)
            throws RecommendationException {
        // Count the favorite game IDs from the database
        // for the given user. E.g. if the favorited game
        // ID list is ["1234", "2345", "2345", "3456"],
        // the returned Map is {"1234": 1, "2345": 2,
        // "3456": 1}
...
//        }
        ...
        if (sortedFavoriteGameIdListByCount.size() > DEFAULT_GAME_LIMIT) {
            sortedFavoriteGameIdListByCount =
                    sortedFavoriteGameIdListByCount
                            .subList(0, DEFAULT_GAME_LIMIT);
        }

        List<Item> recommendedItems = new ArrayList<>();
        TwitchClient client = new TwitchClient();

        // Search Twitch based on the favorite game IDs returned
        // in the last step.
        ...
        for (Map.Entry<String, Long> favoriteGame :
            ...
            List<Item> items;
            try {
                items = client.searchByType(favoriteGame
                        .getKey(), type,
    ...
    // Keys of the may are [Stream, Video, Clip]. Each key is
    // corresponding to a list of Items objects, each item object
    // is a recommended item based on the previous favorite
    // records by the user.
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            if (entry.getValue().size() == 0) {
                ...
                recommendedItemMap.put(
                        entry.getKey(), recommendByTopGames(ItemType
                                .valueOf(entry.getKey()), topGames));
            } else {
                recommendedItemMap.put(
                        entry.getKey(), recommendByFavoriteHistory(
    ...
    // Keys of the may are [Stream, Video, Clip]. Each key is
    // corresponding to a list of Items objects, each item object
    // is a recommended item based on the top games currently on Twitch.
    public Map<String, List<Item>> recommendItemsByDefault()
        ...
        for (ItemType type : ItemType.values()) {
            recommendedItemMap.put(type.toString(),
